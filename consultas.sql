...
select c.id,c.nombre, sum(d.valor_venta) as total_gastado from cliente c left join pedido p on p.id_cliente = c.id left join domicilio d on d.id_pedido = p.id group by c.id, c.nombre having total_gastado > 50000; 
