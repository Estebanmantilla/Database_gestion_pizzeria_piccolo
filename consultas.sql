...
select p.nombre as repartidor, count(d.id_pedido) AS total_pedidos from persona p left join repartidor on r.id = p.id left join domicilio d on r.id = d.id_repartidor group by r.id, p.nombre order by total_pedidos desc; 
